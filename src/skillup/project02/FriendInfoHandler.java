...
public class FriendInfoHandler {
	...
	/*
	 	멤버변수 : 상속관계에 있어 최상위 클래스인 Friend로 배열을 생성한다.
	 		클래스가 상속관계에 있다면 상위클레스 배열에 하위클래스의 인스턴스를
	 		저장할 수 있다.
	*/
	...
	public FriendInfoHandler(int num) {
		...
		// 핸들러 클래스의 인스턴스를 생성할 때 데이터 저장을 위한 Friend타입의 배열의 인스턴스도 함께 생성한다.
	...
	public void addFriend(int choice) {
		...
		/*
		 	1, 2번을 구분해서 각 인스턴스를 생성한 후 저장은 부모타입인 Friend 배열에 저장한다.
		...
		*/
		...
		/*
		 	...
		 	자동형변환이 일어난다. 정수를 실수타입의 변수에 저장할때 자동형변환이
		 	되는것과 동일한 현상이라 생각하면 된다.
	...
	public void showAllData() {
		...
		for(int i=0; i<numOfFriends; i++) {
			/*
			 	instanceof 연산자 : 클래스와 인스턴스 간에 상속관계가 있는지
			 	 확인하여 true/false를 반환해주는 연산자.
			...
			*/
			...
			if(myFriends[i] instanceof UnivFriend) {
				...
				((HighFriend)myFriends[i]).showAllDate();
			}
		}
	...
	public void showSimpleData() {
		/*
		 	저장된 인스턴스의 갯수만큼 반복해서 각 인스턴스가 원래 어떤 타입이었는지
		 	확인 후 원래의 타입으로 형변환한다.
		*/
		for(int i=0; i<numOfFriends; i++) {
			if(myFriends[i] instanceof UnivFriend) {
