...
/*
 	메서드의 규칙
 	- Java에서 main메서드는 public static void로 선언
 	- static으로 선언된 메서드는 static으로 선언된 메서드만 호출할 수 있다.
 	- 메서드 호출 후 반환값은 호출한 위치로 반환된다. 이때 메서드는 메모리에서 소멸된다.
 	- 반환값이 없는 void형 메서드도 실행이 완료되면 호출한 위치로 실행의 흐림이 돌아온다.
 */
public class E01MethodBasic {
	/*
	 	메서드1 : 반환값이 o, 매개변 o => 매개변수로 2개의 정수를 전달받아 합의 결과를 반환한다.
	 		반환값은 호출한 지점으로 반환된다.
	...
	/*
	 	메서드2 : 반환값 x, 매개변수 x => 전달 받는 값 없이 입력받은 후 namePrint() 메서드를 호출한다
	 		이와 같이 반환값이 없는 경우에는 반드시 void를 명시해야한다.
	 */
	public static void menuPrint() {
		System.out.println("당신의 이름은 무엇?");
	...
	/*
	 	메서드3 : 반환값 x, 매개변수 o => 문자열을 인수로 전달받아 단순히 출력한다. 
	 		반환값이 없는 경우 실행이 종료되면 호출했던 지점으로 실행의 흐림이 이동된다.
	 */
	...
	public static void main(String[] args) {
		/*
		 	...
		 	매개변수가 있다면 그 갯수만큼 포함해야한다.
		 	매개변수의 갯수가 다르거나 타입이 틀리면 에러가 발생된다.
		 */
