...
/*
 	지역변수(Local variable)
 	- 변수는 사용범위(Scope)를 가지고 있고, 해당 지역 내에서만 사용할 수 있다.
 	- 해당 지역을 벗어나면 즉시 메모리에서 소멸된다.
 	- 기본자료형은 스택(Stack)이라는 메모리를 사용한다.
 	- 스택은 입구가 하나만 있는 컵과 같은 형태이며, '선입후출'의 특성을 가진다.
...
public class E06LocalVariable {
	...
	/*
	 	main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 메서드는
	 	크기가 동일한 서로 다른 지역에 정의되었으므로 같은 이름의 변수를 선언할 수 있다.
	 	앞의 if~else문의 블럭도 같은 개념으로 변수를 선언한 것이다.
	 */
	...
	public static void main(String[] args) {
		 ...
		 */
//		int num = 9; // 이 부분이 활성화되면 아래에서 에러 발생
		...
		if(scope) {
			...
			int num = 5;
			System.out.println("첫 번째 if문 else지역 : " + num);
		}
