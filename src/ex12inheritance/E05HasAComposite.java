...
/*
 	구성관계로 표현하기에 적합한 Has-A 관계 : X has a Y -> X가 Y를 소유(포함)하다.
 		Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가
 		형성되어 권총이 없는 경찰을 표현하기가 힘들어진다.
 		따라서 구성(Composition)관계로 표현하는것이 적합하다.
 		구성관계란 클래스의 멤버로 다른 클래스의 인스턴스를 사용하는 것을 말한다.
*/
// 권총을 추상화한 클래스
class Gun {
	// 탄환수
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		/*
		 	...
		 	참조변수가 null이라는 것은 참조할 인스턴스가 없는 것을 의미하므로
		 	소유하지 않은 것을 표현할 수 있다.
		*/
		if(bNum <= 0) {
			gun = null;
