...
/*
 	2개의 클래스가 상속관계에 있을 때 가장 기본이 되는 코드는 아래와 같다.
 	디폴트 생성자는 자동으로 삽입되고, 자식쪽에 super()도 자동으로 삽입될 수 
 	있으므로 생략 가능한 코드가 존재한다.
*/
//class AA {
//	public AA() {} // 생략가능
//}
...
/*
 	상속(Inheritance) : 기존에 정의된 클래스에 메서드와 변수를 추가하여 새로운
 		클래스를 정의하는 것을 말한다. 부모클래스를 자식클래스가 상속할 때 
 		extends를 사용한다.
*/
// 부모(상위)클래스 : 사람의 일반적인 형태를 추상화
class Man {
	// 멤버변수는 private이므로 클래스 내부에만 접근 가능
	private String name;
...
// 자식(하위)클래스 : Man클래스를 상속받아 회사원을 추상화
class BusinessMan extends Man {
	...
	public BusinessMan(String name, String company, String position) {
		/*
		 	상속받은 자식클래스는 부모클래스의 생성자를 호출하여 부모인스턴스가
		 	메모리에 먼저 생성되도록 해야한다. super()는 부모클래스의
		 	생성자를 호출해준다. 만약, super()를 기술하지 않으면 컴파일러에
		 	의해 자동으로 삽입되어 디폴트 생성자를 호출하게 된다.
		*/
	...
	public void tellYoutInfo() {
		...
		System.out.println("직금은 " + position + "입니다");
		/*
		 	부모클래스의 멤버변수 name은 private으로 선언되었으므로 상속받은 자식클래스에
		 	접근할 수 없다. private멤버는 무조건 자신의 클래스 내부에서만 접근할 수 있다.
		*/
//		System.out.println("이름은 " + name + "입니다");
		// 직접 접근이 안되므로 public으로 선언된 메서드를 통해 간접적으로 접근해야한다.
		tellYourName();
	}
}
