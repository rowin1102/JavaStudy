...
/*
 	Is-A관계 : X is Y -> X는 Y의 일종이다로 표현가능한 관계
 	- 노트북은 컴푸터의 일종이다.
 	- 휴대폰은 전화기의 일종이다.
...
class Computer {
	...
	// 매개변수가 있는 인자생성자 추가. 이 경우 디폴트 생성자는 추가되지 않는다.
...
/*
 	컴퓨터를 상속한 노트북 : 컴퓨터가 가진 기본 계산기능에 휴대성을 확장하여 정의.
 	휴대하기 위해서 배터리가 필요하므로 멤버변수로 확장.
*/
class NotebookComputer extends Computer {
	...
	public void movingCal() {
		...
		if(battery < 1) {
			// 방전된 상태라면 사용이 불가하므로 return을 통해 해당 메서드의 실행을 중지.
		...
		/*
		 	...
		 	자식클래스에서도 접근할 수 없이 에러가 발생한다. 이 경우
...
/*
 	테블릿 : 컴퓨터의 기본 계산기능과 휴대이용, 전용펜으로 드로잉할 수 있는 기능까지
 		추가된 컴퓨터를 표현
*/
