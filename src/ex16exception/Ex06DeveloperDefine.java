...
public class Ex06DeveloperDefine {
	...
	public static void main(String[] args) {
		...
		try {
			/*
			 	아래 메서드에서 AgeErrorException 예외 인슽턴스를 throw하면
			 	여기에서 예외처리 한다. 즉, 메서드를 호출한 지점에서 예외처리를 하는 방식이다.
			 */
			int age = readAge();
	...
	public static int readAge() throws AgeErrorException {
		...
		try {
			/*
			 	나이 입력시 문자를 잘못 입력하느 경우에는 Java에서 미리 정의해 둔
			 	예외처리 클래스를 사용하면 된다.
			*/
			inputAge = sc.nextInt();
		...
		/*
		 	만약 나이를 음수로 입력하는 경우 JVM이 감자하지 못하므로 이때는 if문을 통해
		 	판단해야 한다. 개발자가 정의한 예외 인스턴스를 생성한 후 직접 throw 한다.
		*/
		if(inputAge<0) {
