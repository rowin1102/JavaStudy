...
import ex14innerclass.Unit.Marin;

/*
 	내부클래스(Inner class) : 클래스 안에 정의된 클래스로 static이 없는 클래스를 말한다.
...
//		medic = new Medic();
	...
	// 내부클래스 정의(내부 멤버 클래스)
	class Marin {
		// 내부클래스도ㅓ 일반적인 클래스와 동일하게 멤버를 정의할 수 있다.
		int attackPower;
		public void attack() {
			/*
...
//		Marin m = new Marin();
		...
		/*
		 	...
		 	Marine은 Unit클래스의 생성자에서 이미 인스턴스 생성이 되어있으므로 이와 같이
		 	바로 사용하면 된다.
		*/
		Unit.Marin marine = new Unit("마린", 50).marine;
		marine.attack();
		
		// Medic은 내부에서 생성되지 않았으므로 new를 두번 사용해서 인스턴스를 생성하면 된다.
