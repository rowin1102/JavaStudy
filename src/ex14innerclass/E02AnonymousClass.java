...
class Singer {
	...
	/*
	 	Object 클래스에 정의된 메서드로 객체가 가진 값을 문자열로 반환해 준다.
	 	Java에서 정의하는 모든 클래스는 Object클래스를 상속하므로 이와 같이 오버라이딩 할 수 있다.
	*/
	@Override
...
/*
 	인터페이스 정의 : 기능이 없는 추상메서드만 멤버로 정의할 수 있다.
 		또한 메서드 앞에는 public abstract가 생략되었다.
*/
interface ISinger {
	public void dancing();
}
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 	인스턴스의 참조값을 할당받은 참조변수를 출력한다. 이때 오버라이딩한 toString의
		 	반환값이 출력된다. 만약 toString을 삭제하면 @를 포함한 참조값이 출력되게 된다.
		*/
		...
		/*
		 	GirlGroup 클래스를 상속받은 익명클래스 정의 : Singer타입의 변수에
		 	GirlGroup을 상속받은 익명클래스를 생성해서 그 주소값을 할당한다.
		...
		*/
		Singer s3 = new GirlGroup("아이린", "레드벨벳") {
			// 자식에서 확장한 멤버변수. 해당 변수는 익명클래스 내부에서만 사용할 수 있다.
			boolean mic = true;
			@Override
			public String toString() {
		...
		/*
		 	...
		 	다운캐스팅 자체가 불가능하다.
		 	따라서 익명클래스에서는 멤버의 확장은 거의 사용하지 않고, 대부분 오버라이딩을 목적으로만 활용된다.
		*/
//		System.out.println(s3.mic);
		System.out.println(s3);
		
		/*
		 	인터페이스를 구현해서 익명클래스를 정의한다. 오버라이딩이 주된 목적이므로,
