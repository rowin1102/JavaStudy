...
/*
 	Call by reference(참조에 의한 호출) : 메서드를 호출할 때
 		참조값을 전달함으로써 서로 다른 지역에서도 동일한 메모리(Heap영역)를
 		참조할 수 있게하는 호출방식이다.
...
public class E04CallByReference {
	...
	static void callByReference(int[] ref) {
		/*
		 	매개변수를 통해 전달받은 배열 인스턴스의 참조값을 통해 0번 원소와
		 	1번 원소를 서로 교환한다. 이때 배열의 값이 서로 바뀌게 된다.
		 */
		int temp;
		temp = ref[0];
		ref[0] = ref[1];
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	출력3 : 200, 100
		 */
		System.out.println("[main메소드]-swap이후 출력");
		for(int i=0; i<arr.length; i++) {
			System.out.println(arr[i] + " ");
		}
