...
/*
 	다형성(Polymorphism) : 하나의 인스턴스(객체)를 여러 타입의 참조변수를
 		통해 참조할 때 참조변수의 타입에 따라 다양한 형태의 인스턴스를 이용할 수 있는
 		특성을 말한다
 	- 부모타입의 참조변수로 자식인스턴스를 참조
 	1. 이 경우 부모타입의 참조변수는 부모로부터 상속받은 멤버까지만 접근할 수 있다.
 	2. 자식에서 오버라이딩한 메서드가 우선적으로 호출된다.
 	3. 자식에서 새곱게 정의한 멤버는 접근할 수 없다.
...
class MyChild extends MyParent {
	...
	@Override
	void parentMethod() {
		System.out.println("자식에게 Overriding한 메소드 parentMehod()");
	}
	...
	*/
	void parentMethod(int childMember) {
		...
		System.out.println("Overloading : 자식에게서 확장한 메소드 : parentMethod(param1)");
	}
}
...
//		parent1.childMember = 1;
		...
		/*
			...
			소괄호를 하나 더 추가해서 myParent 를 자식타입으로 형변환 후 
			멤버변수에 접근해야 한다.
		*/
		((MyChild)myParent).childMember = 1;
		...
		/*
		 	자식타입으로 형변환 이후에 해당 참조변수를 통해 자식 멤버에 접근한다.
		 	위와 동일한 방식이다.
		*/
		...
		/*
		 	...
		 	또한 Object 클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있고,
		 	팔요하다면 오버라이딩도 할 수 있다.
		*/
