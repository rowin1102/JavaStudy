...
/*
 	오버라이딩(Overriding) : 클래스가 상속관계에 있을때 하위클래스에서
 		상위클래스의 멤버메서드와 완전히 동일한 모양으로 메서드를 재정의 하는 것을
 		말한다. 오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히
 		똑같은 형태로 정의된다는 것이다.
...
class Speaker {
	...
	// 멤버변수가 private이므로 외부에서 설정할 수 있도록 setter메서드를 정의
...
// 자식클래스 : 스피커에 베이스의 기능을 보강한 형태로 정의
class BaseSpeaker extends Speaker {
	...
	/*
	 	@Override : 어노테이션이라 부르고, 오버라이딩된 메서드에 추가하여 컴파일러에게
	 		알려주는 역할을 한다. 필수는 아니므로 생략할 수 있다.
	*/
	@Override
	public void showState() {
		/*
		 	super를 통해 부모클래스에 정의된 멤버메서드를 호출한다.
		 	특히 오버라이딩된 메서드를 호출할때 super가 없으면 재귀호출이 되므로 주의해야한다.
		*/
		super.showState();
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		 	...
		 	부모인스턴스로 제한된다. 따라서 자식쪽에 정의된 setBase()는 호출할 수 없다.
		*/
