...
// 해당 클래스는 sub패키지에 작성되었으므로 사용을 위해서는 반드시 import 해야한다.
...
//		one.privateMethod();
		/*
		 	private으로 선언된 멤버에 외부클래스에서 접근하려면 default 혹은 public으로
		 	선언되어 외부접근이 가능한 메서드를 통해 간접적으로 접근해야한다. 클래스 내부에서는 접근지정자의 
		 	영향을 받지 않으므로 해당 메서드를 통해 접근이 가능해진다.
		 */
		one.defaultMethod();
		one.publicMethod();
