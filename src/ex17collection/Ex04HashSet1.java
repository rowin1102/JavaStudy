...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		// size()를 통해 저장된 데이터의 갯수를 알 수 있다.
		System.out.println("[중복 저장전 객체수]:" + set.size());
		/*
		 	기본클래스의 중복저장 : 기본 클래스의 인스턴스인 경우 별도의 오버라이딩 없이도
		 		중복이 제거된다 따라서 아래 add()는 false를 반환한다.
		*/
		...
		while(itr.hasNext()) {
			...
			/*
			 	HashSet에 저장시 모두 Object형으로 자동형변환되어 저장되므로 우리가 직접 정의한
			 	클래스에 대해서만 instnaceof를 통해 타입을 확인한 후 다운캐스팅하여 출력한다.
			 	만약 다운캐스팅을 하지 않으면 자식멤버에 접근할 수 없으므로 getName을 호출할 수 없다.
			*/
			if(object instanceof Teacher) {
				System.out.println("Teacher객체=>" + ((Teacher)object).getName());
			} else {
				// 기본클래스인 경우 별도의 처리없이 출력 가능함
		...
		// 인스턴스 삭제 : set은 인덱스를 사용하지 않으므로 인스턴스의 참조값을 통해 삭제한다
