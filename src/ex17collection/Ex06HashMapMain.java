...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		// Map컬렉션 생성. Value를 모두 String으로 선언
		HashMap<String, String> map = new HashMap<String, String>();
		
		/*
		 	객체저장 : 저장시 기존에 저장된 동일한 key값이 존재하면 저장된 인스턴스가 반환된다.
		 		만약 처음이라면 null을 반환한다.
		*/
...
//		map.put("age", number);
		...
		/*
		 	중복저장 : 기존에 입력된 name이라는 Key가 있기 때문에 '홍길동'이 반환된다.
		...
		*/
		...
		/*
		 	2. Key값을 모를때(혹은 너무 많아서 명시하기 힘들때) : keyset()을 통해 전체
		 	key를 Set으로 얻어온다. 이 값을 통해 반복문으로 접근하여 value를 인출할 수 있다.
		...
		*/
		...
		// 3. 이터레이터 사용 : 위와 동일하게 keySet()으로 얻어온 후 반복 출력한다.
		...
		// values() 메서드를 통해 값만 얻어올 수 있다. 컬렉션의 형태로 얻어온 후 반복해서 출력한다.
		Collection<String> values = map.values();
		for(String value : values) {
			System.out.println(value);
		}
		
		// 객체의 존재유무 확인 : Map은 key와 value가 있으므로 메서드도 2개로 나눠져 있다.
