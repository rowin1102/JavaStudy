...
/*
 	...
 	- Array 라는 이름처럼 '배열'의 특성을 가지고 있어 index를 통해 저장 및 접근이 가능하다.
*/
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	ArrayList와 나머지 컬렉션들은 데이터를 저장하는 내부적인 자료구조만 다를 뿐
		 	사용법은 100% 동일하다.
		*/
...
//		Vector<String> list = new Vector<String>();
		...
		/*
		 	...
		 	add(인덱스, 인스턴스) : 데이터 젖아시 인덱스를 직접 부여한다. 
...
//		list.add(5, "소방차"); => 에러 발생
		...
		*/
		...
		/*
		 	방법2 : 확장 for문의 경우 인덱스 없이 전체 원소를 반복해주므로 좀 더 간단한 표현이 가능하다.
		 		따라서 가장 많이 사용된다.
		*/
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		/*
		 	...
		 	2. hasNext()로 인출한 인스턴스가 있는지 검사한다. 만약 없다면 false를 반환한다.
		...
		*/
		...
		/*
		 	삽입하기 : 해당 인덱스에 인스턴스를 삽입한다.
		 		기존 인스턴스는 뒤로 1칸씩 밀려난다. 즉, 자동으로 인덱싱된다.
		*/
		list.add(4, "블랙핑크 샆입하기");
		for(Object obj : list) {
			System.out.println(obj + " ");
		}
		System.out.println("\n=========================\n");
		...
		/*
		 	포함여부확인 : 특정 인스턴스가 저장되어 있는지 확인할때 사용.
		 		존재한다면 true를 반환한다.
		*/
		...
		/*
		 	...
		 	삭제후에는 전체적인 인덱싱이 자동으로 진행된다.
		*/
		Object obj = list.remove(2);
		System.out.println("삭제된객체 : " + obj);
		
		// 방법2 : 방법1과 동일하게 인덱스로 삭제한다. 단, indexOf로 인스턴스의 인덱스를 찾은 후 이 값을 사용한다.
		int index = list.indexOf("빅뱅");
		System.out.println("빅뱅의 index = " + index);
		list.remove(index);
		
		/*
		 	방법3 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공할 경우 true가 반환된다.
		 	즉, boolean타입의 반환값을 가진다. 
		*/
		...
		// 전체삭제 : 아까 2개의 메서드를 통해 컬렉션에 저장된 전체 데이터를 삭제할 수 있다.
		list.remove(list);
		list.clear();
