...
/*
 	break : 반복문이나 switch문에서 사용되며 블럭내에서 탈출할 때 사용된다.
 		만약 중첩된 반복문이라면 가장 가까운 블럭 하나만 탈출
 	continue : 반복문의 처음으로 돌아간다. while문의 경우 조건을 확인하고,
 		for문의 경우 증감식이 실행
 	위 2개의 문장은 반드시 조건문과 함께 사용되어야 한다. 그렇지 않으면 하위 코드는 실행되지
 	않는 쓰레기 코드가 되기 때문에 에러가 발생
 */
public class E06BreakContinue {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			/*
			 	중첩된 반복문 내부에서 break를 만나면 가장 가까운 반복문 하나만 탈출하게 된다.
			 */
			for(int x=1; x<=5; x++) {
				// x는 1~5까지 전체가 출력됨
