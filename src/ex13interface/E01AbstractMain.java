...
/*
 	...
 	- 추상클래스는 대부분 추상메서드를 포함하고 있으므로, 
 		이를 상속받은 클래스는 반드시 추상메서드를 오버라이딩 해야한다.
...
abstract class AbsClass1 {
	...
	public static final int MAX_IMT = Integer.MAX_VALUE;
...
/*
 	추상클래스2 : 추상메서드를 포함한 클래스. 추상메서드를 멤버로 포함한 클래스는
 		반드시 abstract로 선언해야 한다. 그렇지 않으면 에러가 발생한다.
*/
...
/*
 	위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속받게 되므로 해당 클래스도 abstract를 선언해야 한다.
 	단, 이 경우 해당 클래스로도 인스턴스를 생성할 수 없게된다. 따라서 인스턴스 생성을 위해서는 부모의 추상메서드를
 	자식쪽에서 오바라이딩 하면 된다.
*/
class AbsClassChild2 extends AbsClass2 {
	/*
	 	오버라이딩은 부모쪽의 메서드를 가리고, 생로운 기능을 재정의 하는 것이므로 해당 클래스는 추상메서드를
	 	포함하지 않은 상태가 된다. 따라서 인스턴스 생성이 가능해진다.
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		/*
		 	추상클래스 내에 선언된 정적메서드는 오버라이딩의 대상이 될 수 없으므로 static의 기본규칙을 따른다.
		 	별도의 생성 절차없이 클래스명으로 바로 접근할 수 있다.
		*/
		AbsClass1.staticMethod();
		AbsClassChild1.staticMethod();
		System.out.println("int형의 최대값 : " + AbsClass1.MAX_IMT);
