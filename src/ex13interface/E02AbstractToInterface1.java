...
/*
 	...
 	해당 클래스는 추상메서드를 포함하고 있으므로 하위에서 상속받게 되면 받드시
 	추상메서드를 오버라이딩 해야한다.
*/
...
/*
 	DTO(Data Transfer Objet) : 데이터를 저장하고 전송하기 위한 용도의 객체를 의미한다.
 		VO(Value Object)로 표현하기도 한다.  프로그램에서 이름과 주민번호를 저장하는 용도로
 		사용된다.
*/
...
/*
 	제공된 추상클래스를 상속하여 한 사람의 정보를 저장하는 기능과 검색의
 	기능을 구현한 클래스
*/
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0; i<numOfPerInfo; i++) {
			...
			if(juminNum.compareTo(personalArr[i].getJuminNum()) == 0) {
				...
				/*
				 	DTO의 멤버변수는 private으로 선언됬으므로 외부에서는 접근이 불가능하다.
