...
class SumMulti2 { 
	...
	// 쓰레드의 동기화 방법1 : 동기화 메서드를 사용한다.메서드 전체를 동기화하는 방법이다.
//	synchronized public void addNum(int n) {
//		num += n;
//	}
	
	/*
	 	동기화 방법2 : 동기화 블럭을 사용한다.
	 		동기화가 필요한 코드에만 동기화하는 방법이다. 해당 프로그램에서는 실행코드가 1줄밖에 없으므로
	 		동기화 방법에 대한 성능차이가 없다. 하지만 실행코드가 많은 경우 메서드 전체를 동기화 하는 것보다
...
class MultiAdderThread2 extends Thread {
	...
	/*
	 	...
	 	이 경우 동기화 처리가 되지 않아 동시에 접근되는 경우가 발생하게 되어 잘못된
	 	결과가 출력된다.
	*/
	public void run() {
...
public class Ex05Sync2Method {
	...
	public static void main(String[] args) {
		...
		/*
		 	하나의 인스턴스를 2개의 쓰레드가 공유해서 사용하는 형태로 실행한다.
		 	각각의 쓰레드가 5000번씩 반복해서 호출한다.
		*/
		...
		System.out.println("1~10000까지의 함:" + s.getNum());
	}
