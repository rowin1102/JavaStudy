...
class MessageSendingThread extends Thread {
	...
	public void run() {
		for(int i=1; i<=1000; i++) {
			...
			/*
			 	기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만 sleep()을 호출하면
			 	실행 중 잠깐씩 블럭상태로 전환되어 우선순위가 낮은 쓰레드로 실행될 기회가 생긴다.
			*/
			try {
				sleep(1);
			}
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		 	1단계 실행 : 우선순위를 부여하지 않은 상태로 쓰레드를 생성한다.
		 		이때는 우선순위 5개를 부여받게 된다.
		 		도든 우선순위가 동일하므로 CPU가 작업을 분배한다.
		*/
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/*
		 	2단계 실행 : Thread 클래스의 정적상수를 이용해서 우선순위를 부여한다.
