...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		...
		/*
		 	char형 : 하나의 문자를 저장할 수 있는 자료형으로 값을 저장할 때 '(Single quotation)으로 감싸서 표현한다.
		 		만약 "(Double quotation)으로 감싸면 문자열로 인식하므로 String으로 선언해야한다.
		 */
		char ch1 = '가';
		// 에러 발생. 문자열이므로 char로 선언할 수 없다
		//char chStr = '가나다라';
		// String 타입으로 선언해야함
		String chStr = "가나다라";
		...
		/*
		 	아스키코드 : 1byte로 표현할 수 있는 문자로 영어, 숫자를 십진수로 정의한 것을 말함
		 		EX) A => 65, a => 97
		...
		// 싱글퀘테이션으로 감쌌으므로 문자 1(아스키코드:49)
		char ch4 = '1';
		// 연산 결과는 50이므로 이에 해당하는 문자는 '2'
		char ch4_1 = '1' + 1;
		// 문자 '2'와 정수 50이 출력됨
		System.out.println("ch4 = " + ch4 + ", ch4_1 = " + ch4_1 + ", ch4_1 = " + (int)ch4_1);
		
		/*
		 	boolean타입 : true 혹운 false 두 가지의 값만 가질 수 있는 자료형으로 산술연산(+, -), 비교연산(<, <=)에서는 사용할 수 없고,
		 		논리연산에서만 사용할 수 있다.
		 */
		boolean bn1 = true;
		boolean bn2 = false;
		...
		/*
		 	&& : 논리 And로 양쪽 모두 true일때만 true를 반환
		 	|| : 논리 Or로 둘 중 하나만 true여도 true를 반환
		 */
		// T && F => false를 반환 
		boolean bn3 = bn1 && bn2;
