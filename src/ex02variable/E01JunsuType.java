...
public class E01JunsuType {
	/*
		...
		-일반적으로 CPU(중앙처리장치)는 정수형 연산을 가장 빠르게 처리하는 특징이 있다. 즉 int에 최적화
		-따라서 정수형 변수를 사용할때에는 대부분 int형을 사용하면 된다.
	 */
	public static void main(String[] args) {
		...
		/*
			100 / 200의 결과는 0이 나오게 된다. Java에서는 정수와 정수를 연산하면 정수의 결과가 반환.
		...
		/*
			...
			-하지만 큰 자료형과 작은 자료형을 연산하면  큰 자료형으로 자동 변환. 이유는 데이터의 손실(오차)이 있을 수 있기 때문.
			-단, byte와 short는 예외적인 상황으로 생각하면 된다. CPU가 int형의 연산에 최적화되어 있기 때문
		 */
		...
		/*
			...
			이때 접미사(L or l)을 붙여서 int형의 자료가 아닌 long형의 자료임을 표시해야한다.
			이와 같이 입력되는 숫자도 자료형을 기반으로 메모리에 로드 되는데 이를 가르켜 리터럴(Literal)이라고 한다.
		 */
