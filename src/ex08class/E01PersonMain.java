...
/*
 	추상화(Abstraction) : 현실세계의 사물을 클래스로 형상화하는 것을
 		뜻한다. Person 클래스는 "사람"의 일반적인 사항으로 클래스로
 		추상화하고 있다.
 */

/*
 	클래스 선언시 class 예약어를 사용한다. 클래스명의 첫 글자는 반드시
 	대문자로 기술해야한다. 또한 public 접근지정자는 하나의 Java 파일에
 	한 번만 사용할 수 있다. public으로 선언되는 클래스는 해당 파일명과
 	일치해야한다.
 */
class Person{
	
	/*
	  멤버 변수 : 클래스에서 주로 속성(데이터)를 표현한다. 사람을 추상화한
	  	클래스이므로 이름, 나이 등이 적의되어 있다.
	 */
	String name = "정우성";
	int age = 46;
	String gender = "남자";
	...
	/*
	 	멤버메서드 : 클래스에서 객체의 동작(행위)를 표현한다. 클래스 외부에서
	 		호출시에는 인스턴스 참조변수를 사용해서 .(닷)으로 호출한다.
