...
class FruitSeller{
	...
	/*
	 	멤버변수의 경우 초기값이 없는 상태로 정의한 후 인스턴스 생성 후 초기화할 수 있다.
	 	하지만 멤버상수는 인스턴스 생성과 상관없이 정의하는 시점에 반드시 초기화해야한다.
	 	즉, 1000을 지우면 에러가 발생하게 된다.
	 */
...
class FruitBuyer{
	...
	// 과일 구매를 위한 멤버메서드
	/*
	 	1. 판매자에게 금액을 지불한다. => 여기에서 seller인스턴스의 메서드를 호출한다.
	 	2. 판매자가 금엑에 해당하는 사과의 갯수를 반환하면, 그만큼 보유한 사과가 증가한다.
	 		=> numOfApple += 메서드호출()
	 	3. 판매자에게 지불한 금액만큼 차감된다.
	 */
	public void buyApple(FruitSeller sellr, int money) {
