...
class FruitSeller2{
	...
	/*
	 	두 번째 버전에서는 멤버변수의 초깃값을 제거한다.
	 	이 경우 멤버상수는 에러가 발생하므로 final을 제거하여 일반적이멤버변수로 변경한다.
	...
	// 초기화 메서드를 선언하여 인스턴스 생성 후 다양한 형태로 초기화 할 수 있다.
...
class FruitBuyer2{
	...
	// 멤버면수 초기화를 위한 초기화 메서드 추가
	public void initMembers(int _myMoney, int _numOfApple) {
		myMoney = _myMoney;
		numOfApple = _numOfApple;
	}
	
	public void buyApple(FruitSeller2 sellr, int money) {
		numOfApple += sellr.saleApple(money);
		myMoney -= money;
	}
