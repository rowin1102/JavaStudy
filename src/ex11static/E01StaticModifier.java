...
/*
 	...
 	- main() 메서드가 실행되기 전에 초기화되므로 main() 메서드 실행시 별도의
 		선언 없이 변수를 즉시 사용할 수 있다.
 	
 	접근방법
 	- 클래스내부 : 변수명으로 일반적인 멤버변수와 동일하게 접근할 수 있다.
 	- 클래스외부
...
class MyStatic {
	...
	*/
	int instanceVar;
	/*
	 	정적변수 : 인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수 있다.
	...
	*/
	...
	/*
	 	정적 멤버메서드 : 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	 		정적 멤버만 접근 가능하다.(main 메서드에서는 static으로 선언된
	 		메서드만 호출할 수 있었던 것을 기억하자)
	*/
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 	일반적인 멤버변수에 접근하는 방법 : MyStatic클래스의 인스턴스를
		 		생성한 후 참조변수를 통해 접근한다.
		*/
...
//		System.out.println("MyStatic.staticVar = " + myStatic.staticVar);
		...
		*/
		...
		/*
		 	인스턴스형 변수는 각각의 인스턴스 내부에 존재하므로, 아래 2개의 값은
		 	서로 다르게 출력된다.
		*/
		ms1.instanceVar = 100;
		...
		/*
			...
			어떤 인스턴스 변수로 접근하든 동일한 메모리가 된다. 따라서 값을 공유하게
			되므로 동일한 결과를 출력한다.
		*/
//		ms1.staticVar = 800;
//		ms2.staticVar = 900;
...
//		System.out.println("ms2.staticVar = " + ms2.staticVar);
		...
		/*
			...
			정적변수에 값을 입력한 결과가 되므로, 정적변수에 접근시에는 참조변수를
			사용하지 말고, 클래스명을 사용하는 것이 좋다.
		*/
		...
		/*
		 	정적멤버 메서드에 접근방법은 정적변수와 동일하다. 참조변수를 통해서는
...
//		myStatic.staticMethod();
		...
		/*
		 	클래스명을 통한 접근은 정적메서드만 가능하다. 인스턴스 메서드는 인스턴스 참조변수를
		 	통해서만 접근할 수 있다.
		*/
//		MyStatic.instanceMethod();
		MyStatic.staticMethod();
		...
		/*
		 	...
		 	정적(Static)멤버는 클래스명을 통해 인스턴스 생성 없이 접근한다.
		*/
	}
}
