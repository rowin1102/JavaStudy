...
/*
 	싱클톤 디자인패턴 : 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스
 	즉, 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용할 때 쓰는 패턴
 	
 	작성방법
 	1. 생성자의 접근지정자를 private으로 선언
 	2. 클래스 외부에서는 생성자에 접근할 수 없으므로 new를 통해 인스턴스를 생성할 수 없다.
...
class NoSingleTone {
	int instVar;
	/*
		생성자는 거의 대부분 public으로 선언한다. 막약 public으로 선언하지 않으면
		클래스 외부 혹은 다른 패키지에서는 호출할 수 없기 때문에 new를 통해
...
class SingleTone {
	...
	int shareVar;
	/*
	 	정의한 클래스와 동일한 타입으로 선언된 정적 멤벼변수로 JVM(자바가상머신)에 의해
	 	프로그램이 시작될때 미리 Method영역에 로드되어 사용할 준비를 마치게 된다.
	*/
	...
	*/
	...
	// 멤버변수 출력용 메서드
	void print() {
		System.out.printf(String.format("shareVa = %d\n", shareVar));
	}
}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		// 일반적인 방식의 인스턴스 생성이므로, new로 생성할때마다 새로운 참조값을 할당받게 된다.
