...
public class SQLselect1 extends MyConnection {
	...
	/*
	 	쿼리문 실행시 사용하는 두가지 함수
	 	executeUpdate() : insert, update, delete 쿼리문을 실행할 때 사용.
	 		쿼리실행 후 반환값은 정수로, 적용된 행의 갯수가 반환된다.
	 	executeQuery() : select 쿼리문을 실행한다. 반환값은 조회할 레코드이므로
	 		ResultSet 인스턴스가 반환된다. Java에서 반환받은 후 while문으로 갯수만큼
	 		반복하여 인출한다.
	*/
	@Override
		...
		try {
			...
			while(rs.next()) {
				...
				*/
				...
				/*
				 	날짜를 getString()으로 인출하면 시간까지 포함되어 출력된다.
				 	0000-00-00 00:00:00 형식이므로 적당한 크기로 잘라야 한다면 sunstring()을 사용하면 된다.
				*/
