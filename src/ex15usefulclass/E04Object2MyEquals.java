...
class IntNumber {
	...
	// equals() 메서드가 어떻게 인스턴스 간의 내용을 비교하는지 설명하기 위한 코드를 기술함
	// 비교를 위해 IntNumber타입의 인스턴스를 매개변수로 받은 후
	public boolean myEquals(IntNumber numIbj) {
		// 매개변수로 받은 인스턴스와 멤버변수의 값을 비교한다.
		if(this.num == numIbj.num) {
			// 값이 동일하면 true
			return true;
		} else {
			// 다르면 false를 반환한다.
...
public class E04Object2MyEquals {
	...
	public static void main(String[] args) {
		...
		// 개발자가 직접 정의한 myEquals를 통해 인스턴스간의 내용비교를 한다. 여기서는 다르다고 나온다.
		...
		/*
		 	인스턴스 간의 비교를 할때는 단순히 참조변수에 대한 비교를 하면 안되고,
		 	실제 내용에 대한 비교를 해야한다. 즉, 멤버변수끼리 비교해서 판단해야 한다.
		*/
