...
public class E05Date {
	...
	/*
	 	Calendar 클래스를 통해 얻은 인스턴스를 초(Sec)로 변환한 후 매개변수로 절달된
	 	날짜서식으로 변환한 문자열을 반환한다.
	*/
	...
	public static void main(String[] args) {
		...
		/*
		 	Calendar 클래스는 Java의 대표적인 싱글톤 객체이므로 new를 사용하는 대신
		 	유틸리티 메서드인 getInstance()를 통해 참조값을 얻어온다.
		*/
		Calendar calendar = Calendar.getInstance();
		/*
		 	월을 표현하는 Month, 분을 표현하는 Minute의 첫 글자가 동일하므로 구분을 위해
		 	월은 대문자, 분은 소문자를 사용한다.
		*/
		...
		*/
		...
		/*
		 	...
		 	여기서는 두 가지 서식의 날짜를 파싱하여 구한 날짜 인스턴스로 계산한다.
		*/
		// 오늘날짜 : 0000-00-00 형식
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			 	...
			 	따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여 연산할 수 있다.
			*/
...
//			System.out.println(formatToday + " " + formatFuture);
			...
			/*
			 	...
			 	또한 후루는 60*60*24 죽, 86400초 이므로 아래화 같이 남은 날짜를 계산할 수 있다.
			*/
